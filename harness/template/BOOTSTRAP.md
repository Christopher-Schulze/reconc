...
capped at three consecutive blocks. Install adds `hooks.enabled=true` only when
the field is absent; an explicit user `false` remains installed but disabled.
Restart the ZCode session after installing or removing hooks.
