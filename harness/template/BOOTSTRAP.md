...
Grok Build loads `.grok/hooks/reconc.json` only after project trust is granted with `/hooks-trust` or `--trust`. Its native PreToolUse event is a hard explicit allow/deny boundary, but its native Stop event is passive and Grok treats hook crashes, malformed output, and timeouts as fail-open. The generated wrapper therefore converts Reconc runtime failures into valid Grok deny JSON. Use `reconc grok . --prompt "..."` when strict same-session continuation is required without modifying the Grok binary. When Grok runs in leader mode (`grok --leader` or config `use_leader`), the Stop route additionally steers the live TUI session: it interjects the continuation reason over the leader socket (`x.ai/interject`), capped at 32 attempts per session, never on user interrupts, fail-open, and disabled by `RECONC_GROK_STEER=0`; `reconc doctor --deep` reports reachability as `Grok leader steering`.
...
- When Grok is selected, `reconc doctor --deep` proves project trust and every native Grok route; strict continuation uses `reconc grok`, or leader-mode TUI steering when a Grok leader is running (the `Grok leader steering` doctor row).
