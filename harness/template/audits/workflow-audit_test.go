...
func TestAuditAgentHooksRequireRepoLocalReconc(t *testing.T) {
	...
	writeFile(t, root, ".codex/config.toml", "[features]\nhooks = true\n")
...
const proc = Bun.spawn([binary, "hook", "runtime", event, repo])
const wrapper = repo + "/tools/reconc/bin/hook"
const commandFor = async (event) => {
  return [wrapper, event, repo]
}
const options = { killSignal: "SIGKILL" }
await run("opencode-session-start", payload)
await run("opencode-pre-tool-use", payload)
await run("opencode-permission-request", payload)
await run("opencode-post-tool-use", payload)
await run("opencode-post-tool-use-failure", payload)
await run("opencode-post-compaction", payload)
await run("opencode-session-end", payload)
await run("opencode-stop", payload)
if (event?.type === "session.idle") await handleStop(event)
await client.session.prompt({ path: { id: sessionID } })
`)
...
AGENTS.md
docs/tasks.md
reconc session-briefing . --json (fallback tools/reconc/dist/reconc-<os>-<arch>)
No file writes
_drop/
reconc run on .
`)
	writeFile(t, root, "AGENTS.md", "reconc run on .")

	failures := auditStartEntrypoint(root)
	if !containsFailure(failures, "reconc run off .") ||
