...
func TestAuditAgentHooksRequireRepoLocalReconc(t *testing.T) {
	...
	writeFile(t, root, ".claude/settings.json", `{"hooks":{"SessionStart":[{"matcher":"startup|resume|clear","hooks":[{"type":"command","command":"${CLAUDE_PROJECT_DIR}/`+wrapper+`","args":["claude-session-start","$CLAUDE_PROJECT_DIR"],"timeout":5}]},{"matcher":"compact","hooks":[{"type":"command","command":"${CLAUDE_PROJECT_DIR}/`+wrapper+`","args":["claude-post-compaction","$CLAUDE_PROJECT_DIR"],"timeout":5}]}],"PreToolUse":[{"matcher":"Edit|Write|MultiEdit|Bash","hooks":[{"type":"command","command":"${CLAUDE_PROJECT_DIR}/`+wrapper+`","args":["claude-pre-tool-use","$CLAUDE_PROJECT_DIR"],"timeout":10}]}],"PostToolUse":[{"matcher":"Read|Edit|Write|MultiEdit|Bash","hooks":[{"type":"command","command":"${CLAUDE_PROJECT_DIR}/`+wrapper+`","args":["claude-post-tool-use","$CLAUDE_PROJECT_DIR"],"timeout":5}]}],"Stop":[{"hooks":[{"type":"command","command":"${CLAUDE_PROJECT_DIR}/`+wrapper+`","args":["claude-stop","$CLAUDE_PROJECT_DIR"],"timeout":30}]}]}}`)
