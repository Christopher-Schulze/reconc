...
const createReconcWorkerTransport = (repo, commandFor, runOneShot, canceledMessage) => {
  ...
  const reportContinuation = async (sessionID, state, delivery) => {
    ...
    const event = "kilo-continuation-" + delivery
    await run(event, {
    ...
    if (delivery !== "accepted") {
      console.error("reconc continuation: host=kilo session=" + sessionHash(sessionID) + " route=kilo-stop delivery=" + delivery)
    }
  }
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      await run("kilo-post-tool-use", toolPayload(input, output, "after"))
    },
