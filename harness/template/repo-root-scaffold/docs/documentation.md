...
Reconc governance lives under `tools/reconc/`. The repo-local harness is installed at `tools/reconc/harness/project/` after bootstrap and is wired through `.reconc.yml`, `.githooks/`, `.codex/`, `.github/hooks/`, `.cursor/`, `.agents/`, `.claude/`, `.opencode/`, `.devin/`, `.kilo/`, `.omp/`, `.pi/`, `.grok/`, `AGENTS.md`, `start.md`, and `docs/tasks.md`.
