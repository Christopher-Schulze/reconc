...
func buildRepositoryRunPrompt(state tasklifecycle.RunState) string {
	...
	case tasklifecycle.RunContinue:
		...
		prompt += " Execute Reconc commands yourself. Run `reconc task check-done .` before `reconc task promote .`. Use `reconc run off` only for an explicit user stop. A blocker is recorded automatically; resolve it and run `reconc run on .` to resume."
