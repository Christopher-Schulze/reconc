...
func TestPortableStackAssurancePacksEnforceLiveVerification(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "Next.js",
			preset:     "nextjs-assurance",
			markerPath: "apps/web/package.json",
			marker:     "{\"dependencies\":{\"next\":\"16.0.0\"}}\n",
			sourcePath: "apps/web/app/page.tsx",
			source:     "export default function Page() { return <main>Ready</main>; }\n",
			commands: []string{
				"bun run build",
				"bun run lint",
		...
		{
			name:       "Svelte",
			preset:     "svelte-assurance",
			markerPath: "package.json",
			marker:     "{\"devDependencies\":{\"svelte\":\"5.0.0\"}}\n",
			sourcePath: "src/App.svelte",
			source:     "<main>Ready</main>\n",
			commands:   []string{"bun run build", "bunx sv check --fail-on-warnings"},
		},
		{
			name:       "Zig",
			preset:     "zig-assurance",
			markerPath: "build.zig",
			marker:     "const std = @import(\"std\");\n",
			sourcePath: "src/main.zig",
			source:     "pub fn main() void {}\n",
			commands:   []string{"zig build test", "zig fmt --check ."},
		},
		{
			name:       "Elixir",
			preset:     "elixir-assurance",
			markerPath: "mix.exs",
			marker:     "defmodule Demo.MixProject do\nend\n",
			sourcePath: "lib/demo.ex",
			source:     "defmodule Demo do\nend\n",
			commands:   []string{"mix test", "mix format --check-formatted"},
		},
		{
			name:       "PowerShell",
			preset:     "powershell-assurance",
			markerPath: "Demo.psd1",
			marker:     "@{ ModuleVersion = '1.0.0' }\n",
			sourcePath: "scripts/check.ps1",
			source:     "exit 0\n",
			commands:   []string{"Invoke-Pester -CI", "Invoke-ScriptAnalyzer -Path . -Recurse -EnableExit"},
		},
	}
