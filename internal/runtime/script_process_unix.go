...
func configureScriptProcess(ctx context.Context, cmd *exec.Cmd, done <-chan struct{}, killTimeoutSec int) {
	...
	go func() {
		...
		case <-ctx.Done():
			select {
			case <-time.After(time.Duration(killTimeoutSec) * time.Second):
				if cmd.Process != nil {
