...
func verificationSurfaceAction(kind, surface string) string {
	...
	case KindDSH + ":cli":
		return "Start DSH from the disposable repository with npx --yes @deepseek-ai/dsh@0.1.5-rc.2 --profile headless --patch .dsh/reconc.patch.yml and exercise the native pre-tool guard and final-result observations."
	default:
