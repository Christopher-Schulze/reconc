...
func TestGrokStopWrapperFailsClosedAndPreservesExactBlockJSON(t *testing.T) {
	const fallback = `{"decision":"block","reason":"Reconc could not evaluate this Grok Stop. Repair the Reconc binary or hook installation before retrying."}` + "\n"
	tests := []struct {
		name       string
		binaryBody string
		want       string
		...
		{name: "missing binary", want: fallback},
		{name: "broken binary", binaryBody: "#!/bin/sh\nexit 2\n", want: fallback},
		{name: "clean completion", binaryBody: "#!/bin/sh\nexit 0\n", want: ""},
		{name: "invalid allow", binaryBody: "#!/bin/sh\nprintf '{\"decision\":\"allow\"}\\n'\n", want: fallback},
		{name: "empty reason", binaryBody: "#!/bin/sh\nprintf '{\"decision\":\"block\",\"reason\":\"\"}\\n'\n", want: fallback},
		{name: "exact block", binaryBody: "#!/bin/sh\nprintf '{\"decision\":\"block\",\"reason\":\"finish the task\"}\\n'\n", want: `{"decision":"block","reason":"finish the task"}` + "\n"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			repo := t.TempDir()
			...
			stdout, _, err := runGeneratedWrapper(t, repo, "grok-stop")
			if err != nil {
				t.Fatalf("wrapper error = %v, stdout=%q", err, stdout)
			}
			if stdout != test.want {
