...
grok_fail_closed() {
  if [ "$event" = "grok-stop" ]; then
    printf '%s\n' '{"decision":"block","reason":"Reconc could not evaluate this Grok Stop. Repair the Reconc binary or hook installation before retrying."}'
  else
...
grok_pre_tool_decision_valid() {
  grok_output_is_single_line "$1" || return 1
  printf '%s\n' "$1" | grep -Eq '^\{"decision":"allow"\}$|^\{"decision":"deny","reason":"([^"\\]|\\(["\\/bfnrt]|u[0-9A-Fa-f]{4}))*"\}$'
}

grok_stop_decision_valid() {
  grok_output_is_single_line "$1" || return 1
  printf '%s\n' "$1" | grep -Eq '^\{"decision":"block","reason":"([^"\\]|\\(["\\/bfnrt]|u[0-9A-Fa-f]{4}))+"\}$'
}
...
run_reconc_hook() {
  ...
  if [ "$event" = "grok-pre-tool-use" ] || [ "$event" = "grok-stop" ]; then
    ...
    if [ "$event" = "grok-pre-tool-use" ]; then
      ...
      grok_output=$("$reconc_binary" hook runtime grok-stop "$repo")
    fi
    grok_status=$?
    set -e
    ...
    if [ "$event" = "grok-stop" ]; then
      if [ -z "$grok_output" ]; then
        exit 0
      fi
      grok_stop_decision_valid "$grok_output" || grok_fail_closed
    else
      if [ -z "$grok_output" ]; then
        grok_fail_closed
      fi
      grok_pre_tool_decision_valid "$grok_output" || grok_fail_closed
    fi
    printf '%s\n' "$grok_output"
    exit 0
  fi
...
if [ -f "$target_receipt" ] && [ ! -L "$target_receipt" ]; then
  if exec 3< "$target_receipt"; then
    # shellcheck disable=SC2034 # The second read detects an unexpected extra line.
...
if [ -n "$reconc_target" ]; then
  ...
  if reconc_candidate_trusted "$direct_reconc"; then
