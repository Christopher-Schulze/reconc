...
func RenderText(report Report) []byte {
	...
	fmt.Fprintf(&output, "Action deltas: trace=%d, cache=%d, phase=%d, completeness=%d, reason=%d, tool=%d, failure=%d, approval=%d, ledger=%d.\n",
		...
		report.Summary.ActionLedgerChanges)
