...
func writeCIHelp(output io.Writer) {
	...
	fmt.Fprintln(output, "       reconc ci verify-evidence --evidence FILE --requirement FILE")
	fmt.Fprintln(output, "                 --candidate SHA --candidate-kind commit|merge-queue [--json]")
	fmt.Fprintln(output, "")
	...
	fmt.Fprintln(output, "  Its trusted caller must gate publication on exit zero and use that same candidate")
