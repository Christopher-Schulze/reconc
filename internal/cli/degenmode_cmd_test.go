...
func appendDegenDecisionRaw(t *testing.T, repo string, d agentsession.DegenModeDecision) {
	t.Helper()
	path := filepath.Join(repo, ".reconc", "degenmode", "decisions.jsonl")
...
func TestFollowDegenModeLogTailsNewRecords(t *testing.T) {
	repo := t.TempDir()
	...
	writeDegenDecisions(t, repo, []agentsession.DegenModeDecision{{Event: "stop", Branch: "seed_record"}})
	...
	go func() {
		done <- followDegenModeLog(ctx, repo, "", "", false, 5*time.Millisecond, sw)
	}()

	// Append a NEW record after the follow loop is running.
	time.Sleep(20 * time.Millisecond)
	appendDegenDecisionRaw(t, repo, agentsession.DegenModeDecision{Event: "stop", Branch: "live_tail_branch", Runtime: "cursor"})
...
func writeDegenDecisions(t *testing.T, repo string, ds []agentsession.DegenModeDecision) {
	t.Helper()
	dir := filepath.Join(repo, ".reconc", "degenmode")
...
func TestRunDegenmodeStatusTextAndJSON(t *testing.T) {
	repo := t.TempDir()
	dir := filepath.Join(repo, ".reconc", "degenmode")
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatal(err)
	...
	if err := runDegenmode([]string{"status", repo}, &out, &out); err != nil {
	...
	out.Reset()
	if err := runDegenmode([]string{"status", repo, "--json"}, &out, &out); err != nil {
		t.Fatalf("status --json: %v", err)
	}
	var info agentsession.DegenModeStatusInfo
...
func TestRunDegenmodeLogRenderFilterLimit(t *testing.T) {
	repo := t.TempDir()
	writeDegenDecisions(t, repo, []agentsession.DegenModeDecision{
	...
	var out bytes.Buffer
	if err := runDegenmode([]string{"log", repo}, &out, &out); err != nil {
		t.Fatalf("log: %v", err)
	}
	...
	out.Reset()
	if err := runDegenmode([]string{"log", repo, "--branch", "disable_user_prompt"}, &out, &out); err != nil {
	...
	out.Reset()
	if err := runDegenmode([]string{"log", repo, "--session", "sess-2"}, &out, &out); err != nil {
	...
	out.Reset()
	if err := runDegenmode([]string{"log", repo, "-n", "1"}, &out, &out); err != nil {
...
func TestRunDegenmodeRejectsUnknownSubcommand(t *testing.T) {
	var out bytes.Buffer
	err := runDegenmode([]string{"frobnicate"}, &out, &out)
	if err == nil {
