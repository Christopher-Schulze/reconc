...
func NewRequest(input RequestInput, entropy io.Reader) (Request, error) {
	...
	request := Request{
		...
		SelectedArguments:        append([]SelectedArgument(nil), input.SelectedArguments...),
		BudgetReservationID:      input.BudgetReservationID, ReasonCode: input.ReasonCode,
		RuleIDs: append([]string(nil), input.RuleIDs...), AuthorityPolicyID: input.AuthorityPolicyID,
