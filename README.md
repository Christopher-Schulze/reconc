...
The exact proven matrix uses Reconc built from the checked source, MCP Go SDK `v1.7.0`,
...
| Action policy, trusted context, and budgets | Reconc strictly compiles `actions` and compatible legacy `mcp` authoring into one canonical format-6 action plan, explains and simulates it offline, and routes explicitly configured tools through `reconc mcp gateway` with private keyed identities, crash-safe cumulative budgets, approvals, result inspection, and a decision ledger. |
...
| MCP side-effect control | Classifies explicitly configured Cursor, OpenCode, Kilo, Oh My Pi, DeepSeek Harness, Pi, and ZCode MCP tools as repository reads, writes, commands, or external effects using exact selectors and fail-closed extraction. DSH delivers findings as advisory feedback without blocking host calls. |
...
Inspect retained action decisions without creating missing state:

```bash
reconc action log tail . -n 20
reconc action log stats . --json
...
| Action decision ledger | `$RECONC_HOME/projects/<repository-key>/action/ledger.jsonl*` and `ledger.head.json` | No | Private retained lifecycle evidence, archives, and detached chain head; raw arguments and results are not representable. |
...
| DeepSeek Harness | project-owned `.dsh/reconc.mjs` and `.dsh/reconc.patch.yml`, loaded with `npx --yes @deepseek-ai/dsh@0.1.5-rc.2 --profile headless --patch .dsh/reconc.patch.yml`; non-blocking policy feedback and passive observations, with native/PTC tools, persistent shells, terminals, and delegation available; source-aligned offline regression tests, no qualification run required |
...
Kimi Code CLI, Oh My Pi CLI, and ZCode expose a synchronous Stop event. OMP awaits
...
extension reports requested, locally failed, or suppressed delivery; Pi's
`sendUserMessage` API returns no acknowledgement, so Reconc never claims that
a requested continuation was accepted or completed.
...
Pi loads `.pi/extensions/reconc.ts` only for a trusted project.
`reconc hook install pi .` owns only that file and never edits Pi's trust
store. `reconc hook status .` reports `configured` only when the canonical
...
sets `hooks.enabled=true`, merges only exact Reconc entries under
`hooks.events`, and backs up an incompatible hook subtree before a forced
repair. All seven documented native events are wired. Hard `PreToolUse` blocks
use exit code 2, `PermissionRequest` denials use ZCode's native decision object,
and Stop continuation uses `decision: "block"`; malformed fail-closed requests
use the native exit-code-2 shortcut. Host timeouts remain fail-open. Stop is
capped by the host at three consecutive blocks. ZCode exposes exact tool names
but no MCP server discriminator, so only explicitly configured tool identities
are classifiable.
...
commit. Development builds report `dev` with available VCS metadata; managed
builds include the commit and dirty state. Changed schemas use content-addressed
identities; unchanged contracts retain their published identities.
Version text alone is not release identity; use the exact tag
