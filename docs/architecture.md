...
| `dsh` | `cli` | dsh-session-start, dsh-pre-tool-use, dsh-post-tool-use, dsh-post-tool-use-failure, dsh-stop | documented | Load .dsh/reconc.patch.yml with the selected DSH profile for advisory policy feedback and passive results; no qualification run is required. |
