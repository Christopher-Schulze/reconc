...
| `dsh` | `cli` | dsh-session-start, dsh-pre-tool-use, dsh-post-tool-use, dsh-post-tool-use-failure, dsh-stop | documented | Start DSH from the disposable repository with --patch .dsh/reconc.patch.yml and exercise the native pre-tool guard and final-result observations. |
