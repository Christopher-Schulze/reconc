...
| `dsh` | `cli` | dsh-session-start, dsh-pre-tool-use, dsh-post-tool-use, dsh-post-tool-use-failure, dsh-stop | documented | Load .dsh/reconc.patch.yml with the selected DSH profile and exercise its documented pre-tool, result, and Stop routes. |
