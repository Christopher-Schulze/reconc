...
| Policy script execution | A `require_script` target resolves inside the repository, rejects a symlink leaf, and is opened as one stable executable identity. Its bytes are streamed into an extension-preserving file in a fresh private `0700` directory with the platform-native private ACL and bound by SHA-256; immediately before process creation, source containment plus source/snapshot identity, metadata, and digests are revalidated. Parent/leaf link changes, hard-link or rename swaps, and in-place content replacements therefore fail closed without executing replacement bytes. Arguments, repository working directory, filtered environment, JSON stdin, and platform dispatch remain unchanged. `timeout_sec` is capped at 300 seconds and `kill_timeout_sec` at 60 seconds. An owned caller lifecycle, including MCP request or gateway shutdown cancellation, is the parent of that configured timeout. Caller cancellation returns its context error; only the configured child deadline is reported as a policy-script timeout. On Unix, cancellation sends `SIGTERM` to the complete process group and escalates to `SIGKILL` after the bounded grace; Windows preserves its native immediate process-kill backend. Synchronous CLI and hook surfaces have no independent request lifecycle and use the same bounded configured timeout as their documented background fallback. A shared concurrency-safe sink retains the exact first 64 KiB of stdout and stderr independently; later bytes are discarded without changing the script's exit-status disposition. Every violation message, explanation, and remediation is valid UTF-8 capped at 16 KiB; multi-context failure aggregation retains the first actionable details within 8 KiB and reports the exact number of omitted failures. Structured matched and required evidence remains exact within its independent input limits. |
| Impact Lab | Candidate policy files are capped at 8 MiB; strict replay corpora and full typed JSON reports at 64 MiB, corpora at 10,000 cases, and reviewed action-delta manifests at 8 MiB. Repository filesystem snapshots stop at 100,000 entries, 64 MiB per regular file, and 512 MiB of aggregate regular-file content before hashing; revalidation reuses the same limits while hashing every admitted identity again, so drift detection remains exact and bounded. Corpus and delta-manifest files are accepted only from one stable non-symlink regular-file snapshot whose opened and path identities, size, mode, and modification time still agree after the read. JUnit, SARIF, and GitHub projections retain at most 1,024 findings and 8 MiB. |
| Action approvals and state | Canonical approval objects are capped at 64 KiB, authority registries at 1 MiB, sealed request state at 4 KiB, approval TTL at 120 seconds, future issuance skew at 30 seconds, pending approvals at four, retained approval records at 65,536, and the complete private action state at 16 MiB. Finalization returns one typed outcome: a terminal record is either uncommitted or durably present with its state version. Denial-count exhaustion after a successful persist is a diagnostic on that outcome, not a failed compare-and-swap. Retry and restart recover the existing terminal record without a second reservation transition. |
...
| Action policy evaluation | The MCP gateway starts its 500 ms evaluation context before input normalization and cache lookup. Preparation, selectors, condition nodes, inspection and budget candidates, repository-effect traces, result assembly, and glob matching observe that context; glob programs poll at most every 64 bounded work steps. Deadline or caller cancellation wins at the final publication checkpoint, fails closed, and is never cached. Each operation between checkpoints remains independently bounded by the action input, plan, trace, regex, or matcher limits. |
...
| MCP gateway | Protocol frames and results are capped at 10 MiB, arguments at 8 MiB, discovery at 512 tools across 64 pages and 8 MiB aggregate metadata, concurrent calls and pending approvals at four each, progress at 128 events and 1 MiB per call, retained child stderr at 256 KiB, and serialized operator diagnostics at 4 KiB per line. One Go 1.27 `jsontext` pass validates framing and extracts borrowed envelope fields for observers and progress routing before the SDK consumes the original bytes. Escaping fields are cloned, transformed bytes alone are revalidated, and cleared reader/writer buffers are retained only through 256 KiB. Discovery charges and validates each page before requesting the next, retains only canonical contracts, and therefore holds at most the aggregate catalog plus one bounded in-flight page instead of all raw pages. Every in-flight call observes both its request context and gateway shutdown. Shutdown attempts every detached pending approval in stable call order before the bounded call drain, aggregates safe call-and-phase failures, and reports failures from both stages. Failed shutdown finalization retains only the structured retry state, zeroes detached wire buffers, and is retried by a later `Close` before the identity-key lease is released. A persisted terminal approval remains finalized across retries even when denial-count capacity is exhausted; that secondary condition is written to operator diagnostics after durable cleanup, never treated as an uncommitted mutation. Pure user cancellation is graceful only when no independent cleanup failure occurred; joined cleanup failures remain non-zero at the CLI boundary. Every observed SDK call consumes its pending or request-ID state on completion, cancellation, timeout, or response failure, so no terminal path retains the serialized send lease. Tool icons are limited to 32 fully decoded self-contained PNG or JPEG data URIs, 48 KiB each, 2,048 pixels per side, and 4,194,304 pixels; remote URLs and decompression bombs fail closed. Tool `_meta` is absent or empty because extension semantics are not enforced. Definitions, frames, progress, stderr, and results are validated or inspected before exposure. |
| Auxiliary commands and release inventory | Git, Go, attestation, offline-hook, TASK utility, generated-reference, SBOM, and publication-audit subprocesses use purpose-specific 64 KiB to 64 MiB output ceilings and fail on overflow. Command-proof Git, runtime Git, repository-sync Git, and Grok inspection share one concurrency-safe retained-prefix sink with independent stdout/stderr accounting and caller-specific overflow diagnostics. Release assets are hashed as stable non-symlink regular-file streams, release directories stop after the declared inventory ceiling, and committed manifests, archives, and SBOMs use strict bounded reads. |
...
slowdowns with an unchanged sentinel still fail. In comparison contract v8,
...
Result/baseline v6 retain the package executable SHA-256, and comparison v8
...
source paths. After the closing CPU sentinel, a separate pass executes the same
package patterns with `-test.benchtime=64x` and the same internal repetitions.
RSS comes only from these fixed-work processes; time and per-operation allocation
metrics remain from the timed pass. Every workload sample records
`peak_rss_iterations=64`, distinct from its timed `iterations`. The Go benchmark
runner also performs its standard warmup. RSS describes the full process lifetime, including setup
...
retained-cache measurement. The maximum across all samples remains the gated
value with the same tolerance. Unsupported platform RSS remains zero.
...
regression. Historical v4 Go-driver and v5 precompiled timed-process RSS
measurements remain readable and comparable within their respective formats;
mixed measurement formats are rejected. Refresh requires an actual v6
measurement. The parser accepts direct benchmark text and also
...
a new runner-local v6 baseline from a clean v6 measurement of that exact commit and
the same parameters. It preserves every reference tolerance, rejects an existing
output, and never refreshes the checked baseline implicitly. A historical v4/v5
...
`SetSecurityInfo`, then validated through the opened handle and revalidated
against path identity before returning. No private-state ACL mutation calls a
path-based security API; a fallback path is only used to obtain a handle and is
identity-checked before mutation. Create-capable lock opens are
...
An extension records repository evidence only for its own repository. Calls
associated with another repository continue without being credited to this one.
...
prefix collisions do not count. A failed external inspection contributes at
most 1 KiB of normalized error text and one 4 KiB single-line UTF-8 diagnostic;
admitted stdout is explicitly marked when truncated and terminal controls are
removed before text or JSON rendering. Grok runs PreToolUse before its own
...
the ring in chronological order. `run log --follow` validates the full ring
once, identifies the last record by file identity, byte range, and line digest,
then checks unchanged members without decoding them and reads only appended
suffixes or changed members. Rotation preserves the cursor while its exact file
occurrence remains retained; cursor loss, malformed changes, and truncated
records stop the follower instead of skipping or replaying decisions.
...
- Only policy-authored `require_script` entries execute subprocesses. The
  ...
  directory symlink moves execution outside the repository root. Reconc streams
  that validated identity into a private extension-preserving executable,
  binds the bytes by SHA-256, and revalidates source and snapshot immediately
  before process creation. Repository path or content replacement therefore
  cannot select the launched program. The declared
